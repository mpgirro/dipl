...
class ParserPriorityActorMailbox(settings: ActorSystem.Settings, config: Config) extends UnboundedPriorityMailbox(
    ...
    PriorityGenerator {
        ...
        case ParseUpdateEpisodeData(_,_,_,_) => 1
        case ParseWebsiteData(_,_)           => 2
        case ParseNewPodcastData(_,_,_,_)    => 3 // this produces work, so it should be done with lower work than the processing of podcast/episode data
        case _                               => 4
    })
