...
class ParserPriorityActorMailbox(settings: ActorSystem.Settings, config: Config) extends UnboundedPriorityMailbox(
    ...
    PriorityGenerator {
        ...
        case ParseEpisodeData(_,_,_)          => 2
        case ParseWebsiteData(_,_)          => 3
        case ParseNewPodcastData(_,_,_)           => 4 // this produces work, so it should be done with lower work than the processing of podcast/episode data
        case _ => 5
    })
