...
public interface LuceneMapper {
    ...
    default Document podcastDtoToLuceneDocument(PodcastDTO podcast){
        ...
        if(podcast.getPubDate()        != null){ doc.add(new StringField("pub_date", DateMapper.INSTANCE.asString(podcast.getPubDate()), Field.Store.YES)); }
        if(podcast.getLastBuildDate()  != null){ doc.add(new StringField("last_build_date", DateMapper.INSTANCE.asString(podcast.getLastBuildDate()), Field.Store.YES)); }
